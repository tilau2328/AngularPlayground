...
app.factory('FileService', ['$localStorage', '$http', FileService]);

function FileService($localStorage, $http) {
    
    const list = function(path, callback){
        $http.get( "/files", { params: { "path": path || "" }, headers: { 'Authorization': ( $localStorage.currentUser ? 'Bearer ' + $localStorage.currentUser.token : "") } } )
    ...
    const get = function(file, callback){
        $http.get( "/files/get", { params: { "path": file.path, "file": file.name }, 
                                   headers: { 'responseType': "blob", 'Authorization': ( $localStorage.currentUser ? 'Bearer ' + $localStorage.currentUser.token : "") }, 
    ...
    const upload = function(path, file, ow, callback){
        ...
        $http.post( "/files", fd, { transformRequest: angular.identity, 
                                    headers: { 'Content-Type': undefined, 'Authorization': ( $localStorage.currentUser ? 'Bearer ' + $localStorage.currentUser.token : "" ) } } )
    ...
    const remove = function(path, file, callback){
        $http.delete( "/files", { params: { "path": path, "file": file }, 
                                  headers: { 'Authorization': ( $localStorage.currentUser ? 'Bearer ' + $localStorage.currentUser.token : "" ) } } )
